...
\begin{document}
	...
	\subsubsection{设计目标}
	EasyAda-SM并非专为EasyAda-trust设计，而是被设计为支持部署从简单裸机应用到复杂CVM级别Enclave抽象的通用的安全固件\citess{rubbish}。任何运行在更低特权级的不可信组件可以通过SBI接口获取EasyAda-SM提供的TEE等能力。因此本文对EasyAda-SM提出了以下设计目标。
	\begin{enumerate}
	...
	同时，通过对现有RISC-V主流TEE方案进行深入分析，本文发现不同TEE方案对安全内存管理，使用和隔离机制存在较大差异，在安全启动和中断，I/O隔离等方面的机制和策略则较为共通，如Penglai的SM被设计为实现更灵活和细粒度的安全内存管理，因此需要SM实现安全内存分配能力；Keystone的SM则被设计为一次性内存分配，SM不进行动态安全内存分配，仅负责进行保护；ACE中TSM为了适应CVM二阶段页表的构建，实现了专用的页分配器，充分利用Rust的所有权机制消除了重复分配的隐患。
	
	EasyAda-SM分层设计的核心是统一的安全区域和内存管理层（以下简称安全区域管理层）。安全区域管理层将安全内存相关操作，如创建/销毁安全区域，申请/释放安全内存，隔离安全内存等进行封装，对上层的Enclave管理模块等提供了统一的安全内存管理原语。同时，安全区域管理层将安全内存管理原语依赖的硬件操作划分为内存管理和内存保护两个部分，并且抽象出了统一的内存管理原语和内存保护原语，开发者可以根据硬件环境和业务需求提供具体的原语实现。通过提供统一的内存安全原语，SRMM简化了开发Enclave模块的难度；通过抽象统一的内存管理和保护原语，安全区域管理层实现了不同Enclave管理模式间基础代码的复用和可拓展性，开发者可以在上层无感的条件下复用已有原语或根据场景需求和硬件环境实现新的原语。
	...
	3.异构防御效能：EasyAda-trust基于IOPMP的物理隔离与加速器管理CVM 的机密异构计算架构，能否有效抵御恶意设备的DMA攻击并保障端到端I/O数据的机密性和完整性？
	
	\section{测试软硬件环境}
	
	因为当前RISC-V硬件生态的客观限制，目前量产的RISC-V商用芯片在安全特性上存在明显的路线分化：面向端侧的芯片普遍支持PMP和IOPMP拓展，但是缺乏对H拓展的支持；而面向高性能计算场景的芯片虽然支持H拓展，但是通常使用较复杂的IOMMU和高级中断架构（Advanced Interrupt Architecture，AIA），其高昂的硬件面积开销和基于总线事务的中断延迟抖动等使其难以在对成本与实时性高度敏感的嵌入式SoC中广泛落地。
	
	针对当前RISC-V硬件生态的客观限制，本文采用严格依照RISC-V指令集标准实现的QEMU作为硬件测试环境。
	
	\section{SM生态兼容性测试}
	参考EasyAdaZone的实验设计\citess{rubbish}。为验证系统的生态兼容性，本文在极小化代码修改的前提下，除引导程序与安全监视器外完全复用了既有Keystone与Penglai框架中的全量组件。复用范围涵盖宿主机用户态接口、编译工具链、软件依赖库、测试用例以及受保护侧的TA，并以Linux（内核版本4.15）作为Host OS。在性能评估方面，本文选用Coremark和RV8 benchmark作为基准测试集，并以原生Linux环境的执行表现作为对比基线。
	...
	然而，在适配Keystone框架的测试中，系统性能呈现出显著的差异化衰减：如图\ref{keystone_rv8mark}所示，Miniz与AES的相对性能分别断崖式下跌至约0.40与0.57，而计算密集型的Bigint受影响较小，分析可能是因为Keystone为防御缓存侧信道攻击，在物理层面强制启用了硬件缓存分区隔离机制，严格限制了Enclave可用的末级缓存路数，导致高度依赖数据局部性的算法缓存缺失率剧增。在未启用缓存分析的环境中，横向对比原生环境和EasyAda-SM环境可知，EasyAda-SM引入的绝对软件开销仅为3\%至7\%，这一处于合理区间的微小性能折损，主要源于EasyAda-SM模块化分层抽象所导致的底层执行链路延长。

	
	综合上述交叉验证与基准测试结果表明，本文提出的基于Rust构建的内存安全监视器EasyAda-SM在确立坚实底层信任根的同时，兼具高度的执行效率与架构弹性。具体而言，在纯计算密集型负载下，得益于PMP机制的流水线隐式校验以及Rust语言编译期的零成本抽象特性，该安全架构所引入的运行时额外开销趋近于零；而在处理具备复杂访存行为与高频上下文交互的应用时，EasyAda-SM因模块化分层抽象而产生的绝对软件开销亦被有效控制在1\%至7\%的低损耗区间内。
