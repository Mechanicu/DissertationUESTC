...
\begin{document}
	...
	% 开启中文摘要
	\zhabstract
	随着边缘异构计算的快速发展，嵌入式系统面临严峻的数据安全挑战。机密虚拟机作为保障敏感数据与模型资产的核心技术已被广泛应用。然而，现有商用架构多存闭源局限，传统由C/C++构建的底层固件易受内存破坏漏洞威胁；且在端侧受限环境下，实现安全的异构计算环境仍是机密计算领域的难题。为此，本文基于开源RISC-V架构，设计并实现了一套高安全、低开销的微内核可信为内核虚拟化方案EasyAda-trust。本文主要研究与创新如下：
	
	1.提出基于Rust的内存安全底层固件。 深度结合Rust所有权机制设计页分配器，从编译期根除二次释放等内存隐患；将RISC-V物理内存保护原语抽象为统一安全原语，构筑CVM与不可信组件间的物理级强隔离屏障，确立底层信任根。
	
	2.构建支持CVM全生命周期的微内核虚拟化架构。实施物理资源调度与安全鉴权的深度解耦。Hypervisor仅负责宏观资源调度，而安全区域构建、二阶段页表隔离映射及机密流的两阶段上下文切换，均由可信安全管理器（TSM）隐匿执行；并结合本地证明机制实现了CVM的双向可信启动。
	
	3.设计防DMA攻击的机密异构计算环境。 引入RISC-V IOPMP扩展在总线级实施设备动态鉴权；通过构建加速器管理CVM，利用不可信共享内存与半虚拟化技术打通跨域通信，使机密应用旁路不可信软件中转栈，实现底层加速器在机密环境内零拷贝明文使用。
	
	最后，在QEMU仿真环境下对系统开展了多维度评估。结果表明，本方案生态兼容性优异，核心计算（Coremark与RV8）损耗均低于2\%；得益于静态内存预分配，CVM在系统时钟与访存等核心原语上实现近似零损耗；同时，安全直通架构显著提升了异构I/O吞吐量。本文方案在极致内存安全、异构设备复用与总体计算效能之间实现了理想的架构折中。
	\zhkeywords{Rust，RISC-V，微内核虚拟化，机密计算，可信异构计算}
	
	
	% 开启英文摘要
	\enabstract
	With the rapid development of edge heterogeneous computing, embedded systems face severe data security challenges. Confidential Virtual Machines (CVMs) have been widely applied as a core technology to protect sensitive data and model assets. However, existing commercial architectures mostly suffer from closed-source limitations, and traditional low-level firmware built with C/C++ is vulnerable to memory corruption threats. Moreover, realizing a secure heterogeneous computing environment in resource-constrained edge scenarios remains a formidable challenge in the field of confidential computing. To address these issues, based on the open-source RISC-V architecture, this thesis designs and implements EasyAda-trust, a high-security and low-overhead trusted microkernel virtualization framework. The main research and innovations of this thesis are as follows:
	
	1.Proposing a memory-safe underlying firmware based on Rust. By deeply integrating Rust's ownership mechanism, a page allocator is designed to eradicate memory hazards such as double-free vulnerabilities at compile time. Furthermore, RISC-V physical memory protection primitives are abstracted into unified security primitives to construct a strong physical-level isolation barrier between CVMs and untrusted components, thereby establishing the underlying root of trust.
	
	2.Constructing a microkernel virtualization architecture supporting the full lifecycle of CVMs. A deep decoupling of physical resource scheduling and security authentication is implemented. The Hypervisor is solely responsible for macro-resource scheduling, whereas the construction of secure regions, the isolated mapping of two-stage page tables, and the two-stage context switching of confidential flows are all transparently executed by the Trusted Security Manager (TSM). Combined with a local attestation mechanism, a bidirectional trusted boot for CVMs is achieved.
	
	3.Designing a confidential heterogeneous computing environment resistant to DMA attacks. The RISC-V IOPMP extension is introduced to implement dynamic device authentication at the bus level. By constructing an accelerator-management CVM and utilizing untrusted shared memory alongside paravirtualization technologies, cross-domain communication is bridged. This enables confidential applications to bypass the untrusted software relay stack, achieving zero-copy plaintext access to underlying accelerators within the confidential environment.
	
	Finally, a multi-dimensional evaluation of the system is conducted in a QEMU simulation environment. The results indicate that the proposed framework exhibits excellent ecological compatibility, with the performance overhead of core computations (Coremark and RV8 benchmark) kept below 2\%. Thanks to static memory pre-allocation, the CVM achieves near-zero overhead on core primitives such as system clock and memory access. Meanwhile, the secure passthrough architecture significantly enhances the heterogeneous I/O throughput. The proposed framework achieves an ideal architectural tradeoff among ultimate memory safety, heterogeneous device multiplexing, and overall computing performance.
	...
	然而，随着物联网设备功能的不断扩展，嵌入式技术正面临更加复杂的应用需求和安全问题：（1）在场景方面，多样化的嵌入式应用场景对嵌入式系统的实时性、可靠性和资源利用效率提出了差异化要求，对于关键安全场景，需要确保高实时性、可靠性和安全性；而消费电子更关注低成本和高能效。传统嵌入式系统通常只运行单一操作系统，在需要兼顾多种功能需求时，往往依赖物理硬件的堆叠，从而导致系统成本、复杂性和功耗的显著提升；而基于OpenAMP\citess{openamp}的多操作系统部署方案虽然可以一定程度上兼顾多种功能需求，但是无法很好的确保操作系统间的资源隔离和安全。
	...
	虽然TEE能够防御不可信组件对TEE的篡改或非法访问，但是TEE本身仍然可能存在代码漏洞，并且因为TEE通常具备更高的权限，其中的代码漏洞会造成更大的破坏，严重影响TEE的安全性和系统稳定性。以ARM TrustZone为例，披露的代码漏洞多数由不安全内存访问引发的内存破坏所导致\citess{tee-vul}。而Rust\citess{rust}通过编译时的内存安全检查，可以实现可靠性与高效性的兼得，在尽可能消除代码中不安全内存操作的同时确保性能，非常适合开发内存安全的嵌入式应用。目前已经有部分研究\citess{rustsgx, rust-tz}使用Rust实现了TA以消除内存漏洞，但是目前缺少使用Rust实现软件TCB的相关研究工作。
	
	RISC-V是一种基于经典精简指令集（RISC）原理的开源指令集架构（ISA）。得益于其开放与模块化的架构特性，RISC-V可以在保持简洁性和可靠性的同时支持多种平台，硬件系统可以根据场景需要包含可选的RISC-V标准扩展，甚至实现用户自定义的扩展指令，这使得RISC-V可以支持从嵌入式微控制器到高性能计算的丰富场景。过去几年中RISC-V已经在嵌入式领域得到了广泛应用，主流工具链和操作系统均添加了对RISC-V的支持。虽然目前RISC-V已经完善了硬件辅助的虚拟化扩展（H扩展）并且提供了一些硬件以支持在RISC-V上实现TEE方案，但是相关研究仍在起步阶段。
...
we	
	...
	为了降低开发难度，CPU厂商尝试引入基于CVM抽象的Enclave，降低用户开发成本。AMD SEV\citess{sev}是基于AMD硬件虚拟化扩展的扩展，通过为每个VM指定唯一的运行时标识符ASID，该ID在VM整个生命周期中均存储在内存中不发生变化，确保每个虚拟机仅能访问当前ASID关联的系统资源。同时借助AES加密引擎，使用当前ASID关联密钥对所有读写内存操作进行加解密，其他VM或Hypervisor仅能访问加密后的数据，确保了被保护数据的机密性。
	...
	1.提出并实现了一种基于Rust内存安全特性的模块化底层安全监视器EasyAda-SM：针对传统C语言构建的安全固件易受内存破坏漏洞威胁的局限性，本文采用Rust语言从底层独立设计并实现了EasyAda-SM。在架构抽象中，构建了安全区域与内存管理层，深度结合Rust的所有权模型实现了一套基于页分配器（Page Allocator）的安全内存调度机制，从编译期静态消除了二次释放等内存隐患。同时，将物理内存保护（PMP）机制抽象为统一的内存保护原语，实现了不可信组件与CVM安全内存间的物理级强隔离，为整个系统确立了坚实的信任根基。
	
	2.对基于微内核的EasyAda-Hypervisor引入安全扩展以支持CVM全生命周期管理：为适应机密计算的安全需求，本文对现有的EasyAda-Hypervisor进行了深度的安全扩展与架构增强。通过引入职责解耦机制，将常规的物理内存分配与设备发现保留在Hypervisor层，而将二阶段页表隔离映射、安全区域动态构建及可信上下文切换等核心敏感操作委托给EasyAda-SM处理。结合设计的基于本地证明机制的安全启动链与机密执行流，确保了CVM的CPU上下文及内存视图对不可信Hypervisor的绝对隐匿，从而在不破坏原有架构的前提下，实现了不可信虚拟机与机密虚拟机的安全共存与统一调度。
	
	3.扩展EasyAda-trust架构并构建端侧机密异构计算环境：
	针对端侧异构硬件中支持直接内存访问（DMA）的外设可能绕过CPU隔离机制发起的恶意攻击，本文进一步扩展了EasyAda-trust的系统设计。通过引入RISC-V I/O物理内存保护（IOPMP）扩展，实现了基于设备唯一标识（SID）的外设动态鉴权与细粒度内存访问控制。在此基础上，创新性地提出并构建了专用的“加速器管理CVM”架构，利用I/O半虚拟化技术与安全共享内存映射机制，在严格保障用户AI模型权重及隐私数据机密性与完整性的前提下，成功实现了底层硬件加速器在不可信执行环境与机密执行环境间的时空复用，为端侧异构算力的泛化安全部署确立了完备的技术范式。
	...
	第六章：系统测试与分析:本章在QEMU仿真环境中对EasyAda-trust架构及其核心固件开展了多维度的定量评估与安全性验证。首先，通过Coremark与RV8基准测试，验证了EasyAda-SM具备极高的生态兼容性与近乎零的计算开销；其次，测试了EasyAda-trust系统在多种边界条件下的安全启动流程，证明了双向本地证明与信任链机制的稳健性；最后，基于LEBench与网络I/O基准测试，全面评估了CVM核心原语的执行延迟与安全设备直通性能，验证了本方案在极致内存安全与总体计算效能之间实现的科学折中。
	...
	频繁的虚拟机退出（VM-Exit）是制约虚拟化性能的核心瓶颈之一。为此，H扩展为VS态引入了一套完整的S态寄存器影子副本（如vsstatus、vstvec、vsepc、vsscratch）等。当处理器运行在虚拟化环境（V=1）时，Guest OS对常规S态寄存器的读写操作会被硬件自动重定向至对应的虚拟控制状态寄存器中。这一机制使得客户系统在配置自身上下文时无需陷入HS态，极大地降低了寄存器访问带来的上下文切换开销。
	...
	基于微内核的设计思想，EasyAda相比Linux等宏内核操作系统具备更小的软件TCB，而相比传统的RTOS如FreeRTOS等，EasyAda又具备强大的可扩展性，可以通过扩展系统服务实现与宏内核对等的功能和性能。因此EasyAda通过灵活调整用户态系统服务配置，可以适应边缘到中高端嵌入式等多样化的场景和需求，为云边端场景提供通用的OS底座。
	...
	2.Hypervisor解耦：Hypervisor被划分为运行在EasyAda用户态下的VMM和内核态下的Hypervisor驱动，以尽可能复用EasyAda现有代码和功能。前者负责主要的VM管理工作，包括管理VM生命周期和软硬件资源，模拟虚拟化设备后端，以及管理VM服务线程等；后者作为屏蔽不同架构硬件虚拟化扩展实现的抽象层，对外提供基于虚拟化依赖的硬件原语抽象的统一接口，完成对硬件虚拟化扩展相关寄存器进行配置，以及接收和处理VM中断和异常等。
	
	3.静态分区：Hypervisor采用静态分区策略，虽然该策略缺乏灵活性，但是因为嵌入式场景应用高度定制化的特点，静态分区策略足够满足其功能需求。并且静态分区策略可以简化代码逻辑，缩小Hypervisor的攻击面，这一点在引入CVM支持之后更加重要，因为标准RISC-V CoVE扩展要求SM实现数十个SBI接口以实现对CVM资源的细粒度控制，显著增大了TCB的攻击面，并且导致hyperivsor需要数个SBI调用才能完成一个功能，严重影响了系统的实时性和性能。
	
	EasyAda-Hypervisor通过在EasyAda中引入两个核心组件实现基于硬件虚拟化扩展的VM生命周期管理：（1）运行在内核态（HS态）的Hypervisor驱动，负责抽象虚拟化硬件原语并对上提供虚拟化能力接口。其虚拟化能力主要可以分为以下五类：（1）CPU虚拟化：目标是基于虚拟化专用特权级（RISC-V的VS/VU/HS态），为VM提供符合其运行要求数量的vhart并在隔离的硬件环境中运行VM上下文，在实现操作系统无感迁移到虚拟化环境中运行的同时，避免VM访问和篡改Hypervisor或其他VM的上下文信息；（2）内存虚拟化：目标是基于硬件虚拟化提供的二阶段地址翻译能力，通过动态配置和管理VM的二阶段页表，实现细粒度的，VM无感的强制内存隔离；（3）中断虚拟化：目标是基于硬件提供的中断控制器（包括本地中断控制器和平台中断控制器等），通过中断注入或中断直通等手段，实现VM中断管理的同时，避免VM接收和配置不属于当前VM的中断；（4）时钟虚拟化：目标是基于虚拟硬件时钟，通过虚拟化计时器和虚拟时钟补偿计时器，实现VM无感退出/恢复；（5）I/O虚拟化：目标是基于共享物理内存和虚拟中断，通过为不同VM模拟虚拟设备，在保证VM无感迁移的同时，提升VM的I/O性能和安全性。
	...
	\section{本章小结}
	本章系统性地阐述了构建异构机密计算底座相关的核心技术理论。首先，详细分析了RISC-V架构的特权级模型与陷入委派机制，探讨了包含PMP、Smepmp和IOPMP在内的物理内存保护扩展机制，并论述了硬件虚拟化扩展在微架构层面提升虚拟机运行效率的实现原理。其次，归纳了TEE的基本概念、核心安全特性与主流架构演进范式。针对异构计算场景，分析了信任边界由主机CPU向外部硬件加速器延伸过程中存在的硬件隔离与架构兼容性问题。随后，介绍了当前微内核操作系统EasyAda及其虚拟化方案EasyAda-Hypervisor的架构设计和实现，论证了其基于最小特权原则与统一资源抽象在降低TCB及提升系统可扩展性方面的架构优势。最后，阐述了Rust语言在保障内存安全与线程安全方面的核心机制，并客观分析了其在底层系统编程中的技术优势与潜在安全隐患。
	...
	2.可扩展性：可扩展性同样分为硬件和软件两个方面。从硬件角度，虚拟化方案应当支持用户根据需求引入和整合定制化的硬件，或者引入更高级的指令集扩展（如向量计算扩展等）；从软件角度，虚拟化方案应当支持灵活的部署方案，并且允许用户开发定制化应用。
	...
	EasyAda-trust为了兼顾的兼容性和安全性，要求硬件环境至少应当实现RISC-V硬件虚拟化扩展——H扩展和物理内存保护机制（PMP）。边缘用户可以在一个相对轻量化和精简的硬件环境上运行EasyAda-trust，而需要更高性能或依赖定制化硬件的场景则可以引入增强计算能力的指令扩展和定制化硬件。
	...
	EasyAda-SM的设计目标是在不同硬件平台上对通用的安全固件，通过模块化和分层设计，允许用户根据使用场景引入或移除模块。EasyAda-trust扩展了EasyAda-SM，引入了CVM管理模块等组件并设计实现了基于EasyAda-SM的TSM。
	
	同时，EasyAda-trust扩展了EasyAda-Hypervisor以实现对CVM生命周期的管理。为了简化CVM创建流程，避免不稳定的中间状态增加TEE攻击面并影响其安全性，EasyAda-trust仅在EasyAda-Hypervisor中针对CVM定义了三种无中间状态的动作：转换（promote），执行（run）和销毁（destory）。转换负责将一个已创建但未运行的VM实例通过TSM转换为一个CVM实例；执行负责运行一个CVM实例；销毁则负责停止一个CVM实例并回收其资源。

	进一步的，为了在端侧实现安全的外设管理和机密异构计算环境，EasyAda-trust在设计并实现了专用的加速器管理CVM，并针对无IOMMU的场景进行了优化。

	虽然EasyAda-trust将EasyAda-SM和CVM视为可信的软件TCB，但是可信并不意味着绝对安全，用户在开发过程中可能因为缺乏测试等原因引入破坏TEE安全性的代码漏洞，尤其是不安全的内存访问导致的内存泄漏等问题。针对用户可能引入的代码漏洞，EasyAda-trust对EasyAda-SM和CVM分别采取了不同的方案以尽可能减少代码漏洞或隔离并限制局部代码漏洞，避免单一漏洞导致整个TEE被攻破。EasyAda-SM使用内存安全的编程语言Rust实现以尽可能消除其中因为不安全的内存访问产生的代码漏洞，并通过分层设计，将不安全的内存操作进行封装和限制以降低测试和形式化验证的难度。同时，EasyAda-trust为每个CVM启用了硬件虚拟化扩展，通过二阶段地址翻译实现不同可信组件间的强制内存隔离，避免CVM的安全漏洞影响TEE的稳定性和安全性。具体设计和实现将在后文详细介绍。
	...
	\begin{enumerate}
		...
		\item 可扩展性：EasyAda-SM的核心能力仅能依赖安全保护所需的最小硬件集合以确保对硬件环境的广泛兼容性。为了确保EasyAda-trust开发者可以充分利用不同硬件环境上的资源，EasyAda-SM的能力集合应当具备可扩展性，开发者可以扩展或收缩其能力边界适应不同的硬件环境。
	\end{enumerate}
	...
	EasyAda-SM分层设计的核心是统一的安全区域和内存管理层（以下简称安全区域管理层）。安全区域管理层将安全内存相关操作，如创建/销毁安全区域，申请/释放安全内存，隔离安全内存等进行封装，对上层的Enclave管理模块等提供了统一的安全内存管理原语。同时，安全区域管理层将安全内存管理原语依赖的硬件操作划分为内存管理和内存保护两个部分，并且抽象出了统一的内存管理原语和内存保护原语，开发者可以根据硬件环境和业务需求提供具体的原语实现。通过提供统一的内存安全原语，安全区域和内存管理层简化了开发Enclave模块的难度；通过抽象统一的内存管理和保护原语，安全区域管理层实现了不同Enclave管理模式间基础代码的复用和可扩展性，开发者可以在上层无感的条件下复用已有原语或根据场景需求和硬件环境实现新的原语。
	...
	环境初始化：SM完成自身的内存隔离后，将继续进行两个部分的初始化：SBI运行环境和TSM初始化。该阶段初始化逻辑取决于SM设计：（1）如果将TSM作为SBI固件的扩展，则首先进行SBI运行环境初始化，然后进行TSM初始化，最后由SBI固件转移控制权。（2）若将SBI固件作为TSM的静态库，则由TSM主导初始化流程并调用SBI接口，最终由TSM掌握控制权转移。EasyAda-trust采用方法（2）以确保TSM可以在后续完全控制中断和陷入入口。TSM初始化主要需要完成安全内存初始化，防止SM在系统运行过程中被运行在更低特权级的不可信组件非法访问和篡改，并且确保低特权软件的执行在明确定义的入口点陷入到TSM和SM中。当TSM初始化完成时，SM已经通过硬件实现了与不可信组件的强制隔离，可以安全进入更低特权级转移控制权到Hypervisor。
	...
	鉴于当前RISC-V硬件生态的客观限制，目前量产的RISC-V商用芯片在安全特性上存在明显的路线分化：面向端侧的芯片普遍支持PMP和IOPMP扩展，但是缺乏对H扩展的支持；而面向高性能计算场景的芯片虽然支持H扩展，但是通常集成较复杂的IOMMU和高级中断架构（AIA）。然而，这类复杂组件高昂的硬件面积开销及基于总线事务的中断延迟抖动，使其难以在成本与实时性敏感的嵌入式SoC中广泛落地。。
	...
	在读取系统时钟的测试（ref）中，普通VM与CVM的归一化延迟几乎与物理裸机性能一致。这一优异表现归功于实验环境底层硬件对S态时钟中断扩展的完备支持，允许VM直接在VS态配置定时器并接收虚拟中断，规避了传统架构中需要陷入到M态进行注入的开销，这符合TSM直接将CVM时钟中断委派到VS态进行处理的设计。
	...
	基础I/O测试结果表明，底层设备路由架构对CVM性能具有决定性影响。在传统的全/半虚拟化架构下，为遵循严密的内存隔离模型，TSM阻断了不可信Hypervisor的DMA路径。这迫使CVM采用高开销的安全中转机制：I/O数据必须经历内部密码学加解密、向不可信共享缓冲区的多重拷贝，以及高频触发的TSM安全域上下文切换。此类数据冗余拷贝、微架构缓存失效与纯软件计算的叠加，构成了该模式下I/O性能显著衰减的核心物理代价。
	...
	此外，通过LEBench微基准测试与网络I/O测试对CVM的全生命周期开销进行了深度剖析。数据表明，得益于静态安全内存预分配策略与Sstc硬件扩展的支持，CVM在系统时钟与内存管理等核心原语上实现了近似零损耗的极致性能。而在面临物理隔离边界导致的I/O性能衰减时，本文提出的基于IOPMP扩展的安全设备直通架构，成功绕过了高昂的软件密码学计算与不可信内存中转栈，在捍卫系统极高隔离强度的同时，实现了异构外设的零拷贝明文高速复用。综合而言，本系统在极致的内存安全保障与总体执行效能之间取得了科学且理想的架构折中。
