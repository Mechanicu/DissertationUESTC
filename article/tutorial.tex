...
\begin{document}
	...
	最后，在QEMU仿真环境下对系统开展了多维度评估。结果表明，EasyAda-SM能够有效兼容现有RISC-V TEE方案且核心计算（Coremark与RV8）损耗均低于2\%；得益于静态内存预分配，CVM在系统时钟与访存等核心原语上实现近似零损耗；同时，安全直通架构显著提升了异构I/O吞吐量。本文方案在内存安全、异构设备复用与总体计算效能之间实现了有效的架构折中。
	...
	为了应对嵌入式硬件资源丰富化和应用场景复杂化的趋势，现代嵌入式系统正在从单一操作系统演变为混合关键系统（Mixed Critical System，MCS）以提升系统资源利用率和安全性，降低重复开发成本。现代MCS通过引入虚拟化技术，对物理硬件进行抽象，实现在同一硬件环境上部署多操作系统以应对不同应用场景的差异化需求，并且通过隔离不同运行环境间的内存，CPU和I/O等资源，增强了系统的稳定性和可靠性。虽然虚拟化以以虚拟机监视器（Hypervisor）作为可信计算基（Trust Computing Base，TCB），可以为不同的虚拟机和Hypervisor划分出相互隔离的执行环境，但是通用Hypervisor通常拥有复杂的执行逻辑和庞大的代码量，如Linux KVM大量复用了Linux现有代码和功能，导致其攻击面较大极易被攻击\citess{os_tcb}。因此Hypervisor虽然可以提高系统资源的利用率，但是难以有效保护用户的隐私数据。	
	
	为了确保数据得到有效保护，用于保护敏感代码与数据的可信执行环境（Trust Execution Environment, TEE）在过去十年间已经得到广泛普及。目前所有主流CPU指令集均已推出或支持各自的TEE解决方案（如ARM的TrustZone\citess{trustzone}、Intel的SGX\citess{sgx}，AMD的SEV\citess{sev}，以及RISC-V的Penglai\citess{penglai}和Keystone\citess{keystone}等）以提供一个安全的执行环境，该环境通常被称为飞地（Enclave）或安全世界（Secure World）。TEE的应用场景广泛，可部署在云服务器、移动设备、IoT设备、传感器以及硬件令牌等环境中。
	
	目前嵌入式场景下大部分TEE方案基于ARM TrustZone扩展和GP-TEE\citess{gptee}规范实现，但是这种实现方式存在部分缺陷：（1）闭源性：ARM指令集的闭源特性导致开发者其难以根据需求进行精细化定制；（2）扩展性不足：ARM TrustZone将运行环境分为安全世界（Secure World，SW）和普通世界（Normal World，NW），所有的TA均使用同一个SW，TA间资源隔离主要依赖TEE-OS，TCB较大，容易引入代码漏洞或被攻破，并且两个世界间的资源难以在运行时动态调整，系统灵活性较差；（3）开发成本高：该框架下必须开发专用的可信操作系统（TEE-OS）并基于TEE-OS开发可信应用（Trust Application），开发门槛和难度较高。而边缘计算的发展进一步放大了开发成本，端侧AI需要处理大量用户隐私数据，必须得到足够的保护。但是将这些模型完整移植到资源受限并且运行专有TEE-OS的TEE中，或者仅使用TrustZone保护关键计算路径并通过复杂的软硬件逻辑保护上下文或I/O数据，都极大的增加了可信应用的开发难度。
	
	为了在保证可信应用安全性的同时充分复用现有软硬件方案，实现降低TEE的准入门槛和厂商开发可信应用成本的目标，主要CPU厂商均提出了将TEE技术与虚拟化技术相结合，即基于VM抽象的TEE方案（如ARM CCA\citess{cca}，Intel TDX\citess{tdx}等）。通过将Hypervisor移出TCB并且在TEE中直接运行未经修改的机密虚拟机（Confidential Virtual Machine，CVM），未经修改的可信应用可以直接运行在TEE中，极大降低了厂商开发成本。
	
	但是现有基于VM抽象的TEE方案主要面向服务器场景，无法直接拓展到资源和成本严格受限的嵌入式场景。并且，基于VM的TEE方案因为引入未经修改的VM导致TCB剧增，显著增加了TEE中的代码漏洞数量和攻击面，严重影响系统整体的安全性和稳定性。目前披露的TEE中的代码漏洞多数来源于非内存安全语言C/C++编写的程序因为不安全内存访问引发的内存破坏\citess{tee-vul}，为了尽可能消除代码中的安全漏洞，需要尽可能消除TCB尤其是运行在最高特权级的安全固件中的不安全内存访问行为。
	
	Rust\citess{rust}是一种内存安全的语言。通过编译时的内存安全检查，可以兼顾嵌入式场景对可靠性，安全性，实时性和性能的要求。目前已经有部分研究\citess{rustsgx, rust-tz}使用Rust实现了TA以消除内存漏洞，但是目前缺少使用Rust实现软件TCB的相关研究工作。
	...
	综上所述，结合开源，可拓展的RISC-V指令集，实现支持运行机密虚拟机的TEE方案，并且消除TCB中存在的内存漏洞，在确保嵌入式系统实时性和可靠性的同时，提高嵌入式系统的安全性和资源利用率，降低TEE开发成本和准入门槛这一课题具有重大研究意义。
...
we	
	...
	此外，针对TEE内部TA因使用C/C++等非内存安全语言而引入的内存漏洞风险，RusTEE\citess{rust-tz}提出了一套基于Rust语言的框架。该方案通过扩展Rust编译器以适配TrustZone特殊环境，并设计了受信任的系统调用与跨世界通信接口，利用Rust的所有权与借用检查机制在编译阶段静态消除内存破坏隐患，显著增强了TEE软件栈的稳健性。
	...
	随着端侧人工智能与云端大模型业务的爆发式增长，保障异构计算平台（如CPU+GPU/NPU）的数据隐私与模型完整性已成为系统安全领域的研究热点。在架构实现上，高性能服务器场景通常采用独立显存的离散式GPU，TEE仅需隔离GPU的内存映射I/O（MMIO）区域；而在端侧场景中，集成式硬件加速器通常与CPU共享物理内存，因此不仅需隔离MMIO，更要求对GPU缓冲区实施动态物理隔离。为在不改变底层硬件设计的前提下构建GPU TEE，学术界主要探索了基于CPU安全特性监控与软硬协同原语防护两条技术路径。

	在基于CPU安全特性的监控机制研究中，Wu等人\citess{x86_gputee}提出的GEVisor利用现有Intel SGX构建了经过形式化验证的安全参考监视器，在Enclave与GPU间建立可信I/O路径，有效消除了系统对不可信驱动的依赖。面向端侧统一内存架构，Deng等人\citess{arm_gputee}在StrongBox中利用ARM TrustZone技术，通过动态拦截MMIO与二阶段页表控制实现了高效隔离，其可信计算基（TCB）仅约1.4K LoC。为突破传统TEE因频繁数据加解密导致的性能瓶颈，Fan等人\citess{x86_gputee1}提出的XpuTEE框架在VMX Root模式下引入微型监控器XM，通过联合抽象与直接物理映射实现了“去加密化”监控，将大模型推理开销显著压缩至2.5\%以下。
	...
	\begin{figure}[!h]
		...
		\label{second_tcb}
	\end{figure}
	2.机密虚拟机TCB：该部分为EasyAda-trust新引入的TEE架构，用于弥补非机密虚拟机TCB在防御不可信VM攻击时攻击面过大和安全性不足的缺陷。
	...
	EasyAda-SM并非专为EasyAda-trust设计的安全固件方案，其设计目标是在不同硬件平台上针对不同TEE场景提供统一的安全底座。通过模块化和分层设计，EasyAda-SM支持用户根据使用场景复用已有模块设计实现新的Enclave管理模块。本文基于EasyAda-SM设计实现了CVM管理模块。为了避免不稳定的中间状态增加EasyAda-SM攻击面增加，本文仅在CVM管理模块中针对CVM定义了三种无中间状态的动作：转换（promote），执行（run）和销毁（destory）用于EasyAda-Hypervisor管理CVM生命周期。因为嵌入式场景对功耗和成本的要求，通常不会实现IOMMU用于限制具备DMA的恶意设备绕过CPU侧的内存隔离访问安全区域，本文在CVM管理模块中引入了IOPMP管理模块用于防御DMA攻击\citess{dma_attack}。
